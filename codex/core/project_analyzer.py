...
class ProjectAnalyzer:
    ...
    def get_summary(self) -> str:
        ...
        lines = [
            f"📂 项目: {info.name}",
            f"📍 路径: {info.path}",
            f"🏷️ 类型: {info.type or '未知'}",
        ]
        
        if info.language:
            lines.append(f"💻 语言: {info.language}")
        if info.framework:
            lines.append(f"🏗️ 框架: {info.framework}")
        if info.build_tool:
            lines.append(f"🔧 构建工具: {info.build_tool}")
        if info.test_framework:
            lines.append(f"🧪 测试工具: {info.test_framework}")
        
        lines.extend([
            f"📊 文件数: {info.files_count}",
            f"🌿 Git: {'✅' if info.has_git else '❌'}",
            f"🐳 Docker: {'✅' if info.has_docker else '❌'}",
            f"🧪 测试: {'✅' if info.has_tests else '❌'}",
            f"⚙️ CI/CD: {'✅' if info.has_ci else '❌'}",
        ])
